...
const BACKUP_FONT_SETTING := SETTING_PREFIX + "backup_custom_font"
...
func _register_project_settings() -> void:
	...
	# omnemoji/enabled - Toggle plugin on/off
	_add_setting(
		SETTING_ENABLED,
		true,
		TYPE_BOOL,
	...
	# omnemoji/auto_download - Automatically download missing fonts
	_add_setting(
	...
	# omnemoji/emoji_provider - Dropdown for emoji font
	# Index: 0=Custom, 1=System, 2+=provider IDs
	_add_setting(
	...
	# omnemoji/custom_emoji_font - Custom emoji font path (only used when provider = 0)
	_add_setting(
		SETTING_EMOJI_FONT,
		"",
		TYPE_STRING,
	...
	# omnemoji/text_provider - Dropdown for text font
	_add_setting(
	...
	# omnemoji/custom_text_font - Custom text font path
	_add_setting(
		SETTING_TEXT_FONT,
		"",
		TYPE_STRING,
